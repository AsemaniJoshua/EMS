...
try {
    ...
    $now = new DateTime();
    $start_time = new DateTime($registration['start_datetime']);
    $end_time = new DateTime($registration['end_datetime']);
    ...
    $stmt->bindParam(':registration_id', $registration['registration_id']);
    $stmt->execute();
    ...
    $questionsQuery = "
        ...
        ORDER BY " . ($registration['randomize'] ? 'RAND()' : 'q.sequence_number ASC, q.question_id ASC');
    ...
    foreach ($questions as &$question) {
        $choicesQuery = "
            ...
            ORDER BY " . ($registration['randomize'] ? 'RAND()' : 'choice_id ASC');
    ...
    $duration_seconds = $registration['duration_minutes'] * 60;
